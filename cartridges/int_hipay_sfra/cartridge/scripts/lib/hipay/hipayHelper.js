var ArrayList              = require('dw/util/ArrayList'),
    ...
    statuses               = require('~/cartridge/config/hipayStatus').HiPayStatus;
...
HiPayHelper.prototype.fillHeaderData = function(HiPayConfig, order, params) {
    ...
    var threshold            = HiPayConfig.hipayEnable3dSecureThresholdRule,
        ...
        hipayForm            = session.forms.billing.hipayMethodsFields;
...
/* Handles the form responses */
HiPayHelper.prototype.handleFormResponse = function(configs) {
    var updDataObj = {};

    for(var config in configs) {
        var cofigKeys = config.split('_');

        if (empty(updDataObj[cofigKeys[0]])) {
            updDataObj[cofigKeys[0]] = {};
        }

        updDataObj[cofigKeys[0]][cofigKeys[1]] =  configs[config];
    }
